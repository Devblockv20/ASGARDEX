import { cast, flow, IMapType, Instance, SnapshotIn, SnapshotOut, types } from 'mobx-state-tree'
import { naturalTokenToToken } from 'thorchain-info-common/build/helpers/naturalTokenToToken'
import { ICLP, IExchangePair } from 'thorchain-info-common/src/interfaces/metrics'
...
export interface ITokens extends Instance<IMapType<typeof Token>> { }
...
export const Store = types.model({
  ...
  .actions(self => ({
    ...
    fetchTokens: flow(function* fetchTokens() {
      try {
        const clps: ICLP[] = yield http.get(
          `${env.REACT_APP_LCD_API_HOST}/clps`,
        )

        for (const clp of clps) {
          self.tokens.put(clp.clp)
        }
      } catch (error) {
        // tslint:disable-next-line:no-console
  ...
  .actions(self => ({
    afterCreate() {
      setInterval(self.sub, 1000)
      self.sub()
      self.fetchTokens()
    },
  }))
  .views(self => ({
    naturalTokenToToken(amount: number, denom: string): number {
      if (!self.tokens.has(denom)) { return 0 }
      return naturalTokenToToken(amount, self.tokens.get(denom)!.decimals)
    },
  }))
